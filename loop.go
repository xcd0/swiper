...
func LoopPinCheck(ch chan struct{}, q chan struct{}) {
	...
	var lastRecieved time.Time     // 最後に受理した時刻。
	var changeSetting bool = false // 設定変更があった
	for {
		time.Sleep(time.Millisecond * 1) // 1msごとにチェック。
		if IsChatteringDetected() {      // チャタリングの影響を除去する。
			continue
		}

		if prestate == 0 {
			if s.Now != 0 {
				log.Printf("0->1: %v", DebugPrintGPIO())
				...
				if check(s.setting.PinSetting.InputDit) { // 短音入力ピン。
					...
					for {
						...
						if !check(s.setting.PinSetting.InputAny) { // 任意入力ピン。ストレートキー用ピン。
							...
							log.Printf("straightKeyStart: release: %v", d)

							// とりあえず、現在の設定のWPMで3tick以上長押しされていたらdash扱い、それ未満なら短点扱い。
							if d < s.dit*time.Duration(3) {
								rb.Add(Push_InputDit)
							} else { // 長音入力ピン。
								rb.Add(Push_InputDash)
							}
				...
				// 何もしない。
				//log.Printf("0->0: %v", DebugPrintGPIO())
			}
		} else {
			...
			if s.Now == 0 {
				// 何かが押されている状態で、何も押されていない状態になった。
				// ストレートキーのみリリースを検出する必要があるが、ストレートキーは独自でチェックしているのでここでは処理しない。
				log.Printf("1->0: %v : released : %v", DebugPrintGPIO(), time.Now().Sub(lastRecieved))
				if rb.Count() > 0 {
					str, _ := DecodeMorse(rb, ModeReadMorse)
					log.Printf("decoded: %v", str)
				}
			} else {
				if xor := prestate ^ s.Now; xor != 0 {
					log.Printf("1->1(追加) : %v", DebugPrintGPIO())
					// 何かが押されている状態で、別の何かが追加で押された。(同時押し)
					// 同時押しは現状設定値変更処理のみ。
					if CheckChangeSetting(prestate) {
						// 設定変更ピンが押されていたので設定値を更新した。
						changeSetting = true // 設定変更があった。次のモールス信号の打鍵までは設定変更をflashに書き込まない。
					}
				} else if xor == 0 {
					log.Printf("1->1(維持) : %v", DebugPrintGPIO())
					// 何かが押されている状態で、押された状態が維持された。(長押し)
					// 現状長押しで何か効果がある入力は、ストレートキーのみだが
					// ストレートキーのみリリースを検出する必要があるが、ストレートキーは独自でチェックしているのでここでは処理しない。
				}
			}
		}
...
func CheckChangeSetting(prestate PinState) bool {
	...
	if pre_fn1 { // 速度。初期値20WPM。
		...
		for time.Now().Before(end) {
			for {
				...
				if time.Now().Sub(last) > s.dit*time.Duration(4) {
					// 文字受理。
					if rrb.Count() != 0 {
	...
	}
	{
		...
		s.dit = time.Duration(1000*60/50/wpm) * time.Millisecond         // 1つの短音の長さ。
		s.dash = time.Duration(int(float32(s.dit) * s.setting.DashRate)) // 1つの長音の長さ。
	}
	return true
}

func IsChatteringDetected() bool {
	// チャタリングの影響を除去する。
	s.Now = ReadGPIO()
	prestate := s.Now
	...
	if s.Now = ReadGPIO(); prestate == s.Now {
		if prestate == 0 && s.Now == 0 {
			// 何も押されていなかった。
			return false // 何も押されていない、ということも受理する。
		} else {
...
// チャタリングを除去しながら、
// リングバッファrbに蓄積する。
func old_LoopPinCheck(ch chan struct{}, q chan struct{}) {
